...
import AnimationOperations from "../islands/Animation/Operation.tsx";
...
function CardInline(
  ...
  return (
    <li
      id={index % 2 ? "slide-right" : "slide-left"}
      class={`opacity-0 flex flex-row gap-3 rounded-lg bg-base-100 p-3`}
...
function Card(
  ...
  return (
    <div
      id={"slide-left"}
      class={`opacity-0 flex flex-col gap-2 md:w-[calc(50%-0.75rem)] lg:w-[calc(33.3333%-7.9rem)] relative
          ${
        index < 3 &&
        "lg:after:content-[''] lg:after:w-full lg:after:absolute lg:after:h-[1px] lg:after:top-8 lg:after:left-12 lg:after:border-t lg:after:border-t-[#a4aec633]"
      }`}
      style={{ animationDelay: `${index * .3 + 1}s` }}
    >
      <div
        class={`rounded-full w-16 h-16 flex justify-center items-center text-center mb-5 ${
          index < 3 ? "bg-secondary" : "bg-base-100"
        }`}
      >
...
export default function Operation(props: Props) {
  ...
  return (
    <AnimationOperations>
      <div
        id={id}
        class="w-full bg-primary mb-16 flex flex-col gap-7 md:gap-12 h-full py-8 mt-8 mb- after:border-l-[100vw] after:border-l-primary relative after:border-b-[4rem] after:-bottom-[3.995rem] after:border-transparent after:absolute after:bg-neutral after:-z-10 after:left-0 after:right-0"
      >
