...
function CardInline({ title, content, index }: { title: string, content: HTMLWidget, index: number }) {
    return (
        <li id={index % 2 ? "slide-right" : "slide-left"} class={`opacity-0 flex flex-row gap-3 rounded-lg bg-base-100 p-3`}
...
function Card({ title, content, index }: { title: string, content: HTMLWidget, index: number }) {
    return (
        <div id={"slide-left"} class={`opacity-0 flex flex-col gap-2 md:w-[calc(50%-0.75rem)] lg:w-[calc(33.3333%-7.9rem)] relative
